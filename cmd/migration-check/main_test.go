...
// prevReleaseSchemaCols is the (hand-constructed) previous release's "all
// columns of table X" set for the fixtures in testdata/prevrelease -- avoids
// routing every TestExtractReferences case through parseSchemaColumns just
// to get a known column list.
...
func TestExtractReferences(t *testing.T) {
	t.Run("InsertEvent's 14-column list is entirely high-confidence", func(t *testing.T) {
		...
		extractReferences("queries/events.sql", readPrevReleaseFixture(t, "events.sql"), prevReleaseSchemaCols, refs)
		cols := []string{
		...
		for _, c := range cols {
			if _, ok := refs.hasHigh("events", c); !ok {
				t.Fatalf("expected (events, %s) in the high-confidence set", c)
			}
		}
	})
	...
	t.Run("ON CONFLICT target list is high-confidence", func(t *testing.T) {
		...
		for _, c := range []string{"event_type", "source", "external_id"} {
			if _, ok := refs.hasHigh("events", c); !ok {
				t.Fatalf("expected (events, %s) in the high-confidence set (ON CONFLICT target)", c)
			}
		}
	})
	...
	t.Run("UpsertArtist EXCLUDED and table-qualified columns are high-confidence", func(t *testing.T) {
		refs := newPrevReleaseRefs()
		extractReferences("queries/artists.sql", readPrevReleaseFixture(t, "artists.sql"), prevReleaseSchemaCols, refs)
		for _, c := range []string{"mbid", "name", "deezer_id"} {
			if _, ok := refs.hasHigh("artists", c); !ok {
				t.Fatalf("expected (artists, %s) in the high-confidence set", c)
			}
		}
	})
	...
	t.Run("alias-qualified star expands to every schema column, alias.col resolves via the alias map", func(t *testing.T) {
		refs := newPrevReleaseRefs()
		extractReferences("queries/artists.sql", readPrevReleaseFixture(t, "artists.sql"), prevReleaseSchemaCols, refs)
		if _, ok := refs.hasHigh("artists", "image_url"); !ok {
			t.Fatal("expected (artists, image_url) present via a.* expansion / qualified WHERE ref")
		}
		if _, ok := refs.hasHigh("watchlist", "artist_id"); !ok {
			t.Fatal("expected (watchlist, artist_id) present via the alias map (w.artist_id)")
		}
		for _, c := range prevReleaseSchemaCols["artists"] {
			if _, ok := refs.hasHigh("artists", c); !ok {
				t.Fatalf("a.* must expand to every previous-release artists column; missing %s", c)
			}
		}
	})
	...
	t.Run("single-table bare SELECT * expands to every schema column", func(t *testing.T) {
		...
		for _, c := range prevReleaseSchemaCols["events"] {
			if _, ok := refs.hasHigh("events", c); !ok {
				t.Fatalf("SELECT * FROM events (single table) must expand to every events column; missing %s", c)
			}
		}
	})
	...
	t.Run("CTE name is excluded from the real-table set; its inner bare column resolves to the real table", func(t *testing.T) {
		refs := newPrevReleaseRefs()
		extractReferences("queries/events.sql", readPrevReleaseFixture(t, "events.sql"), prevReleaseSchemaCols, refs)
		if _, ok := refs.hasHigh("events", "track_count"); !ok {
			t.Fatal("expected (events, track_count) present from the CTE body's own SELECT track_count FROM events")
		}
		if _, ok := refs.hasHigh("existing", "track_count"); ok {
			t.Fatal("the CTE name \"existing\" must never resolve to a migratable object")
		}
	})
	...
	t.Run("subquery is flattened: bare WHERE column inside EXISTS(...) resolves to the single real table", func(t *testing.T) {
		refs := newPrevReleaseRefs()
		extractReferences("queries/events.sql", readPrevReleaseFixture(t, "events.sql"), prevReleaseSchemaCols, refs)
		if _, ok := refs.hasHigh("events", "artist_id"); !ok {
			t.Fatal("expected (events, artist_id) present via subquery flattening")
		}
	})

	t.Run("sqlc.arg/narg and @param names are collected as parameters, never as columns", func(t *testing.T) {
		refs := newPrevReleaseRefs()
		extractReferences("queries/params.sql", readPrevReleaseFixture(t, "params.sql"), prevReleaseSchemaCols, refs)
		for _, p := range []string{"artist_id", "cutoff", "page_size", "set_release_types", "release_types", "id"} {
			if !refs.params[p] {
				t.Fatalf("expected parameter %q collected into refs.params", p)
			}
		}
		if refs.hasLow("events", "page_size") {
			t.Fatal("page_size must never be asserted as a column, not even low-confidence")
		}
		if _, ok := refs.hasHigh("events", "page_size"); ok {
			t.Fatal("page_size must never be asserted as a column of any table")
		}
	})

	t.Run("bare unqualified column inside a two-table join is low-confidence, not high-confidence", func(t *testing.T) {
		refs := newPrevReleaseRefs()
		extractReferences("queries/low_confidence_join.sql", readPrevReleaseFixture(t, "low_confidence_join.sql"), prevReleaseSchemaCols, refs)
		if !refs.hasLow("widgets_a", "status") && !refs.hasLow("widgets_b", "status") {
			t.Fatal("expected \"status\" in the low-confidence set for at least one joined table")
		}
		if _, ok := refs.hasHigh("widgets_a", "status"); ok {
			t.Fatal("a bare unqualified column in a multi-table join must never be high-confidence")
		}
		if _, ok := refs.hasHigh("widgets_b", "status"); ok {
			t.Fatal("a bare unqualified column in a multi-table join must never be high-confidence")
		}
	})
	...
	t.Run("schema-qualified table name resolves to the bare table", func(t *testing.T) {
		refs := newPrevReleaseRefs()
		extractReferences("queries/schema_qualified.sql", readPrevReleaseFixture(t, "schema_qualified.sql"), prevReleaseSchemaCols, refs)
		if _, ok := refs.hasHigh("events", "title"); !ok {
			t.Fatal("expected public.events to resolve to table events")
		}
		if _, ok := refs.hasHigh("events", "notified_at"); !ok {
			t.Fatal("expected public.events's alias-qualified WHERE column to resolve to table events")
		}
	})
}

func TestExtractReferences_IdentifierCaseFolding(t *testing.T) {
	refs := newPrevReleaseRefs()
	extractReferences("queries/case_folding.sql", readPrevReleaseFixture(t, "case_folding.sql"), prevReleaseSchemaCols, refs)

	for _, lookup := range []string{"image_url", "IMAGE_URL", "Image_Url"} {
		if _, ok := refs.hasHigh("artists", lookup); !ok {
			t.Fatalf("expected an unquoted reference to fold: lookup %q against a stored image_url reference failed", lookup)
		}
	}

	if _, ok := refs.hasHigh("artists", `"Mixed"`); !ok {
...
// trailing alias" regex let the alias group swallow the immediately
// following clause's own FROM/JOIN keyword (`FROM t\nJOIN` -> "JOIN"
// captured as t's alias), which then made FindAllStringSubmatch skip the
// real second table entirely. findFromJoinTables must find both.
