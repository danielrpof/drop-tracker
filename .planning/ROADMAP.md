...
- [ ] **Phase 22: Scheduled Digest Send** - the digest scheduler and batched send: cadence fire times, missed-tick catch-up, DST/tzdata correctness, the last-sent watermark
- [ ] **Phase 23: Digest Readability & Discord Limits** - grouping, the "since <timestamp>" window header, and multi-message chunking that never truncates
...
- *Failure posture for the settings read.* Decide explicitly and test it: a `settings.Get` error should fail **closed toward today's behavior** (real-time) or fail closed toward no-send — the planner picks, but it must be a deliberate, tested branch, not an implicit `false` zero value. Whichever way it goes, a settings read must never wedge or error out the poll cycle itself (`NotifyPending`'s error is already log-and-continue at `poller.go`'s call site — keep that contract).
...
**Requirements**: DGST-05, DGST-06, DGST-07, DGST-08, DGST-09, DGST-15
**Success Criteria** (what must be TRUE):

  1. With digest mode on and events of all three types pending, one Discord message arrives at the next scheduled fire carrying all of them, and every event that message covered is marked delivered afterwards.
...
- *Restart catch-up is a UX decision the planner must lock* (RESEARCH gap #4): on boot, does a detected gap fire an immediate catch-up send, or does it wait for the next natural check and log a warning? Either satisfies DGST-05 because outbox-state windowing loses nothing; decide it explicitly and write it into the phase context rather than letting the implementation imply it.
- *Batching.* This phase delivers the batched send (DGST-08) at whatever fidelity the existing `discord.Client.Send` shape supports — generalizing `Send(ctx, Embed)` (`internal/discord/client.go:110`) to a batch form and reusing `internal/notifier`'s existing `formatEmbed`/`truncateRunes` helpers. Grouping, the window header, and multi-message chunking are Phase 23; a flat chronological batch is acceptable here. Respect the existing 400ms `defaultSpacing` between sends.
...
**Goal**: A digest is something an operator actually wants to read — grouped, stamped with the window it covers, and complete even when it is large enough to exceed what one Discord message can hold.
**Depends on**: Phase 22 (there has to be a digest before it can be formatted or chunked)
**Requirements**: DGST-10, DGST-11, DGST-12
**Success Criteria** (what must be TRUE):

  1. Events inside a digest are grouped under headings (by event type and/or by artist) rather than presented as one undifferentiated flat list — an operator can find "what's new from one artist" without reading every line.
  2. Every digest message states the window it covers ("since <timestamp>"), so a digest arriving after a late, skipped, or caught-up tick is unambiguous about what it includes.
  3. A digest large enough to exceed Discord's per-message embed count or character budget is delivered as multiple ordered messages, spaced by the existing inter-send delay — with no event silently dropped and no content truncated away without a visible marker.
  4. Events are acked per delivered message, not per digest run: a failure partway through a multi-message digest leaves the undelivered remainder pending for the next digest instead of losing it or re-sending what already went out.

**Plans**: TBD

**Notes for the phase planner**

- *Grouping hierarchy is a design decision, not a research finding* (RESEARCH gap #2). Research leans toward compact per-event lines in an embed Description over a field-per-event layout, because Description lines scale much further before hitting limits. Lock the hierarchy (by event type, by artist, or type-then-artist) during this phase's discuss/spec pass.
