...
- [ ] 11-02-PLAN.md — Bounded Deezer fan-out, plus the empirical proofs that concurrency preserves the per-source rate ceiling, the overlap guard, and per-artist error isolation (PERF-02, PERF-03)
- [ ] 11-03-PLAN.md — `AdvanceGroupTrackCountBaseline` atomic compare-and-set replacing the two-statement deluxe-change baseline read/write, re-derived detection branching, and the lost-update race test (PERF-04)
- [ ] 11-04-PLAN.md — Folded-in flaky-test fix: schema isolation for the destructive migrate-from-scratch test and a deterministic spacing seam in the notifier, so criterion 4's `-race` suite half is trustworthy
