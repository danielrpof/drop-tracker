...
- [ ] 02-02-PLAN.md — Add-path completeness: 409 duplicate via SQLSTATE 23505, optional initial preferences with allow-list validation, request-size and field-length bounds

**Wave 3** *(blocked on Wave 2)*

- [ ] 02-03-PLAN.md — `GET /watchlist` stably ordered with `[]` on empty, and `DELETE /watchlist/{id}` hard delete with honest 404 under concurrency

**Wave 4** *(blocked on Wave 3)*

- [ ] 02-04-PLAN.md — `PATCH /watchlist/{id}` independent preference axes with partial updates, CHECK-constraint backstop proof, phase-closing gate
