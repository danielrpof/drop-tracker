...
1. **Does `vitest run` (no `--coverage` flag) actually invoke coverage collection when `coverage` is configured but `test.coverage.enabled` is not explicitly set to `true`?** — **RESOLVED: resolution deferred to empirical verification, executed as the first step of Plan 09-02 Task 1.**
   ...
   - **Why this cannot be answered from research alone:** Vitest 4.1.10's activation behavior for config-only coverage is not stated unambiguously in its published docs, and the docs' own example is ambiguous rather than contradictory. The only authoritative answer is observed behavior of the pinned version against this repo's actual `vitest.config.ts` — i.e. it requires execution, not further reading.
   - **Resolution path (already planned, not deferred to an unowned future):** Plan 09-02 Task 1 step 3 resolves this empirically *before moving on*: add the `coverage` block, run `pnpm --dir web test`, and read the output. If a per-file coverage table prints, coverage activates from config presence alone and nothing further is needed; the executor records that observed finding in `09-02-SUMMARY.md`. If no table prints, the executor adds `enabled` set to true as a fifth sub-key of the `coverage` object and re-runs until the table appears. Either way the fix lands in `web/vitest.config.ts` — never in `web/package.json`'s `test` script and never in the CI step — so D-08's "single unchanged `pnpm test` invocation" holds under both outcomes.
   - **Why the ordering is safe:** Plan 09-02 is Wave 1 (`depends_on: []`), and this check is the first step of its first task. Every downstream consumer of the answer is gated behind it — Plan 09-02 Task 2 carries the `<precondition>` "`pnpm --dir web test` from Task 1 prints a per-file coverage table", so no task can act on an unverified assumption. Both branches of the outcome have a pre-specified one-line remedy, so neither branch can block the phase.
   - Tracked as assumption A2 in the Assumptions Log above (MEDIUM risk, mitigated by the same Task 1 step 3 check).
