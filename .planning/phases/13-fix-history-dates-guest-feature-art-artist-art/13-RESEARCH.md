...
A significant finding not called out explicitly in CONTEXT.md's Decisions: `EventItem`'s TypeScript type **already** exposes `release_date` (`web/app/lib/api.ts:21`), and the Go `Event` struct already serializes it for every event type (`internal/events/service.go:41`). CONTEXT.md's flagged uncertainty here is resolved — no `api.ts` change is needed for D-04/D-05. Similarly, `artists.deezer_id` and `artists.image_url` columns **already exist** in the schema (`000002_watchlist.up.sql`) — the original pass needed no new migration for the artist-art bug itself, only a new sqlc query and a backfill/add-time write path. The post-research grilling round (Q4, see the addendum below) adds exactly one migration anyway, for an unrelated reason: bounding repeated backfill cost across restarts via a new `art_match_attempted_at` column, not because the core match/write path needed schema changes.
...
### Pitfall 1: Comparing MusicBrainz partial dates as plain strings is (usually) safe — but know why, and know where plain `<` actually breaks (amended by the grilling round, Q2)
...
**How to avoid:** Plain lexicographic string comparison of two non-empty MusicBrainz partial dates is correct **only when the two dates differ in their leading 4-digit year, or share the same year at equal precision** — every value starts with a 4-digit year, so a genuine year difference decides ordering outright, and at equal precision within a year (`"2020-03"` vs `"2020-01"`) plain `<` on the remaining digits is correct too. **The original write-up in this doc overstated the safety of plain `<`:** when two dates share a year but differ in *precision* — one is a strict string-prefix of the other (`"2020"` vs `"2020-01-05"`) — plain `<` picks the *shorter, vaguer* value as "earliest" purely because prefixes sort first, not because it's actually earlier. A reissue or compilation commonly carries only a year, while the original release carries a full day; under plain `<`, the vaguer reissue date would incorrectly "win" over the original's precise, genuinely-earlier date. The corrected rule (now D-02): compare years first; on a same-year prefix relationship, prefer the *more precise* (longer) value; only fall back to plain `<` when neither is a prefix of the other. **Always filter out empty-string dates before comparing** (an empty string sorts first lexicographically, which would incorrectly "win" as earliest).
**Warning signs:** A guest_feature event's rendered date is suspiciously always the release with the least-precise date, or a comparison including an undated release silently produces a null date when a real dated release was available. The specific same-year-precision-mismatch failure mode has no natural warning sign in normal operation — it silently produces a plausible-but-wrong date — so it needs an explicit test case (`"2020"` vs `"2020-01-05"`, same year), not just the different-year cases the original examples covered.
...
## Grilling Round Addendum (post-research challenge pass)

A `/grill-with-docs` challenge pass against the finished plans (13-01/13-02/13-03) surfaced six gaps this RESEARCH.md's original pass did not analyze. Each is now a locked decision in 13-CONTEXT.md (D-10–D-13, plus amendments to D-02/D-07/D-08) and is summarized here so a reader of RESEARCH.md alone still has the full picture:

| Q | Gap found | Resolution | Locked as |
|---|-----------|------------|-----------|
| Q1 | Add-time matching (`Service.Add`) and the startup `Backfill` sweep share the same rate-limited `dzClient`/`mbClient` instances; nothing analyzed contention between them, which is guaranteed right after every deploy | `artistart.ActivityGate`: `Backfill` yields with a bounded backoff whenever an add-time match is in flight. A fully separate rate budget was considered and rejected — it would let the backfill exceed MusicBrainz's real external ~1 req/sec ceiling rather than share it | D-10 |
| Q2 | D-02's "earliest date" via plain lexicographic string comparison silently picks a same-year *vaguer* date over a same-year *more precise* (and possibly genuinely earlier) one, because prefixes sort first | Compare years first; on a same-year prefix relationship, prefer the more precise value; plain `<` only decides ties at equal precision | D-02 (amended) |
| Q3 | No diacritic folding in `normalizeArtistName` (this project's own genre scope — reggaeton/Latin R&B — is exactly the population most likely to differ only by accents), and no feedback signal on actual production match rate | Hand-rolled Latin diacritic fold table (no new dependency); `Stats.MatchRatePercent()` in the backfill's summary log, with a documented "investigate normalizer if persistently <~40%" expectation | D-08 (amended), D-11 |
| Q4 | D-07's "one-time" backfill re-sweeps every fail-closed artist on **every** process restart, with no persisted marker — costly given this project's explicit CI/CD-frequent-redeploy purpose | New `artists.art_match_attempted_at` column (migration `000005`) and a 24-hour cooldown predicate on `ListArtistsMissingImage`; recorded via a new minimal `RecordArtMatchAttempt` write, even on a fail-closed outcome | D-07 (amended), D-12 |
| Q5 | A single newly-added, feature-heavy artist's seed cycle can serialize many `ReleasesForRecording` calls against the *same* process-wide MusicBrainz limiter every other concurrently-dispatched artist also needs, stretching a poll cycle's wall-clock duration | Per-artist, per-cycle cap (`maxNewGuestFeatureLookupsPerCycle = 20`); overflow recordings are simply left unattempted this cycle (never marked seen) and naturally retried next cycle | D-13 |
| Q6 | D-08's tie-break requires exact normalized title equality, which real-world edition suffixes ("(Deluxe)", "(Remastered 2020)") would almost always defeat — the tie-break path was likely to (by construction) almost never resolve anything | Tie-break title comparison loosened to equality-or-substring-containment, gated by a minimum normalized length to reject spurious short-substring collisions; does not weaken D-09's fail-closed posture | D-08 (amended) |

The **one** invariant this addendum changes from the original research pass: this phase is no longer migration-free. D-12 (Q4) adds exactly one new migration (`000005_artists_art_match_attempted_at`). Every other "no new dependency" / "no new external package" claim in this document is unaffected — D-11's diacritic fold and D-10's `ActivityGate` are both hand-rolled, zero-dependency additions, consistent with the Package Legitimacy Audit above.
