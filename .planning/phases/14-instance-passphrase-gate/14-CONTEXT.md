...
- Per-IP login throttling + a global brute-force counter that alerts to Discord
- The React SPA's global 401 interception → passphrase screen, plus a logout control
...
- **D-06:** **Sliding renewal.** Cookie `Max-Age` 30 days; when a request arrives past the halfway mark (cookie older than ~15 days), re-issue a fresh 30-day cookie. Absolute cap ~90 days from first login (3× the window), after which re-auth is required regardless of activity. Active devices never get surprise-logged-out; abandoned sessions still expire.
...
### Logout
- **D-10:** Logout is **client-local only** and that is accepted. `DELETE /session` responds with `Set-Cookie … Max-Age=0`, clearing the cookie on that browser. A cookie already copied elsewhere stays valid until its TTL expires. The revoke-all lever at this scale is rotating `INSTANCE_PASSPHRASE` (D-01). No server-side revocation list, no key-version bump. (Documenting the rotation lever in a runbook was offered and not taken — keep it minimal.)
...
### Brute-force defense
- **D-12:** **Per-IP throttle + global counter + Discord alert.**
  - Per-IP: `golang.org/x/time/rate` keyed on client IP, ~5 attempts/min then `429`, plus a fixed ~250ms–1s delay on *every* login response (success or fail) to blunt timing analysis and rapid guessing.
  - Global: a process-wide failed-attempt counter; past a threshold in a window it fires a Discord alert through the existing `internal/discord` webhook path ("possible brute-force on <instance>"), and may apply a short endpoint cooldown. Defends distributed guessing of the one shared secret and reuses the notifier sink (on-brand for the DevOps portfolio). — **Reversibility:** reversible — a small self-contained counter + one wiring point.
...
- **D-14:** Use `chi/middleware.RealIP` (reads `X-Forwarded-For` / `X-Real-IP`) for the per-IP throttle and the audit log. This is safe **only** if the app is never reachable except through the reverse proxy. Phase 14 adds a load-bearing code comment + a runbook note: *the container port must not be published on the VPS — proxy only.* Phase 17 enforces that topology. The "parse only the proxy's appended hop" and "`RemoteAddr` only" alternatives were considered and rejected.
...
- `internal/config/config.go` — add `InstancePassphrase string \`env:"INSTANCE_PASSPHRASE"\`` (empty = gate disabled); follow the existing grouped-by-phase struct convention
...
- `.env.example` — document `INSTANCE_PASSPHRASE` with a strength recommendation
...
- Keep the new configuration surface to exactly one variable (`INSTANCE_PASSPHRASE`) — session timings hardcoded (D-07).
